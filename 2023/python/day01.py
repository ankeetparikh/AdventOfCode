def solve1(s):
  s = s.strip().split("\n")
  d = [[x for x in line if x.isdigit()] for line in s]
  d = [int(a[0] + a[-1]) for a in d]
  print(sum(d))
def solve2(s):
  words = list("123456789") + ["one", "two", "three", "four", "five", "six", "seven", "eight", "nine"]
  wm = { w:(i%9 + 1) for i, w in enumerate(words) }
  def num(t):
    a = min((p, wm[word]) for word in words if (p:=t.find(word)) != -1)[1]
    b = max((p, wm[word]) for word in words if (p:=t.rfind(word)) != -1)[1]
    return a * 10 + b
  s = s.strip().split("\n")
  print(sum(num(line) for line in s))
def solve(s):
  solve1(s)
  solve2(s)
