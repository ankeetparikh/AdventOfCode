def solve1(s):
  ...
  for i in range(n):
    for j in range(m):
      if not s[i][j].isdigit() and s[i][j] != '.': p.add((i, j))
  t = 0
  for i in range(n):
    for j in range(m):
      if (j == 0 or not s[i][j - 1].isdigit()) and s[i][j].isdigit():
        k = j
        while k < m and s[i][k].isdigit(): k += 1
        v = int(s[i][j:k])
        k -= 1
        for x, y in p:
          if (x == i and min(abs(y - j), abs(y - k)) == 1) or (abs(x - i) == 1 and ((j <= y <= k) or min(abs(y - j), abs(y - k)) == 1)):
            t += v
  print(t)
