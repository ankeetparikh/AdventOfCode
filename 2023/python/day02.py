def solve1(s):
  ...
  for line in s:
    ...
    for st in sets:
      arr = st.split(",")
      for item in arr:
        u, v = item.strip().split(" ")
        if v == "red" and int(u) > r: po = False
        if v == "blue" and int(u) > bl: po = False
        if v == "green" and int(u) > g: po = False
    if po:
      t += int(a.split(" ")[1])
  print(t)
...
def solve2(s):
  ...
  for line in s:
    ...
    for st in sets:
      ...
      for item in arr:
        u, v = item.strip().split(" ")
        if v == "red": r = max(r, int(u))
        if v == "blue": bl = max(bl, int(u))
        if v == "green": g = max(g, int(u))
    t += r * bl * g
  print(t)
def solve(s):
  solve1(s)
