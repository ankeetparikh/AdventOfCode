# sval is the S's pipe type
# input manually so that handling S in part 2 is easy
def solve(sval, s):
  s = [list(x) for x in s.strip().split("\n")]
  n, m = len(s), len(s[0])
  si, sj = [(i, j) for i in range(n) for j in range(m) if s[i][j] == 'S'][0]
  f = {(i, j):(i, j) for i in range(n) for j in range(m)}

  def find(c):
    f[c] = c if f[c] == c else find(f[c])
    return f[c]
  def merge(c, d):
    x, y = find(c), find(d)
    if x != y: f[x] = y
  for i in range(n):
    for j in range(m):
      if j + 1 < m and s[i][j] in "-LFS" and s[i][j + 1] in "-7JS": merge((i, j), (i, j + 1))
      if i + 1 < n and s[i][j] in "|F7S" and s[i + 1][j] in "|LJS": merge((i, j), (i + 1, j))
  
  loop = set(k for k in f.keys() if find(k) == find((si, sj)))
  print("Part 1:", (len(loop) + 1) // 2)
  ...
  for i in range(n):
    for j in range(m):
      ...
      for k in range(j + 1, m):
        if (i, k) not in loop: continue
        if s[i][k] == '|': cross, cur = (cross + 1, "")
        if s[i][k] == "L": cross, cur = (cross, "L")
        if s[i][k] == "J": cross, cur = (cross + (cur == "F"), "")
        if s[i][k] == "7": cross, cur = (cross + (cur == "L"), "")
        if s[i][k] == "F": cross, cur = (cross, "F")
      if cross % 2 == 1: t += 1
  print("Part 2:", t)
