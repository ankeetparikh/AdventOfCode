...
def solve1(s):
  ...
  def go(mp, nm):
    if nm == 'A': return True
    if nm == 'R': return False
    rus = wk[nm]
    for ru in rus:
      if ':' not in ru: return go(mp, ru)
      co, nru = ru.split(':')
      t = eval(co, mp)
      if t: return go(mp, nru)
  ans = 0
  for rt in r:
    z = rt[1:-1].split(",")
    mp = {}
...
def solve2(s):
  ...
  def go(mp, rus):
    ...
    ru = rus[0]
    if ru == 'A': 
      return prod(r - l + 1 for l, r in mp.values())
    if ru == 'R': return 0
    if ':' not in ru: return go(mp, wk[ru])
    co, nru = ru.split(':')
    va, bd = co[0], co[2:]
    bd = int(bd)
    x = copy.deepcopy(mp)
